...
class SRT extends GanttChart {
   Comparator<Program> myComparator;
   public SRT(int n, int q) {   //총 프로그램 수 저장
   ...
   protected void pointOfTime(Vector<Program> pv, int totaltime) {
      ...
      DrawGantt display = new DrawGantt(n, totaltime, "SRT");
         ...
         for(int i = 0; i < pv.size(); i++) {   //현 시간에 도착할 프로그램을 레디큐에 넣음
            if(pv.get(i).inTime <= time) {
               ...
               if(runP != null) {
                  ...
                  if(readyQv.get(0).programName.compareTo(runP.programName) == 0) {
                     readyQv.remove(0);
                     runP.leftTime++;
                  }
                  else {
